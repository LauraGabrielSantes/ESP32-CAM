 ...
 * 
 * Se ha agregado al codigo la configuracion para encender un led conectado al pin 13 del ESP32-CAM
 * 
 * Componente     PinESP32CAM     Estados lógicos
 * ledStatus------GPIO 33---------On=>LOW, Off=>HIGH
 * ledFlash-------GPIO 4----------On=>HIGH, Off=>LOW
 * led------------GPIO 13---------On=>HIGH, Off=>LOW
 */
...
int LedPin = 13;//Esta variable controla el led que se agrega en el pin 13
...
void setup() {
  ...
  pinMode (LedPin, OUTPUT);// Se configura el pin extra como salida
  ...
  digitalWrite (LedPin, HIGH); //Se enciende el led extra
  }
...
void loop() {
  ...
  if ((timeNow - timeLast > wait) && statusLed == 0){// Comprobar el encendido del led
    digitalWrite (LedPin, HIGH);// Encender el led
    statusLed = 1;//Indicar que el led  se encuentra encendido
    timeLast = millis (); // Inicia el control de tiempo
  }
}// Fin de void loop
